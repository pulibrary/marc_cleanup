...
RSpec.describe 'invalid_subfield_code?' do
  let(:record) { MARC::Record.new_from_hash('fields' => fields, 'leader' => leader) }
  let(:leader) { '01104naa a2200289 i 4500' }

  context 'record has an invalid subfield code' do
    let(:fields) do
      [
        { '245' => { 'ind1' => ' ',
                     'ind2' => ' ',
                     'subfields' => [{ 'X' => 'invalid subfield' }] } }
      ]
    end
    it 'triggers an error' do
      expect(MarcCleanup.invalid_subfield_code?(record)).to eq true
    end
  end

  context 'record has no invalid subfield code' do
    let(:fields) do
      [
        { '245' => { 'ind1' => ' ',
                     'ind2' => ' ',
    ...
    it 'does not trigger an error' do
      expect(MarcCleanup.invalid_subfield_code?(record)).to eq false
    end
  end
end

RSpec.describe 'invalid_indicators?' do
  let(:record) { MARC::Record.new_from_hash('fields' => fields, 'leader' => leader) }
  let(:leader) { '01104naa a2200289 i 4500' }

  context 'field has invalid indicator1' do
    let(:fields) do
      [
        { '245' => { 'ind1' => 'a',
                     'ind2' => ' ',
    ...
    it 'triggers an error' do
      expect(MarcCleanup.invalid_indicators?(record)).to eq true
    end
  end

  context 'field has invalid indicator2' do
    let(:fields) do
      [
        { '245' => { 'ind1' => ' ',
                     'ind2' => 'a',
  ...
  context 'field has no invalid indicators' do
    let(:fields) do
      [
        { '245' => { 'ind1' => '0',
                     'ind2' => '0',
    ...
    it 'does not trigger an error' do
      expect(MarcCleanup.invalid_indicators?(record)).to eq false
    end
  end
end
